...
impl Game {
    ...
    pub fn run_frame(&mut self) {
        if self.reset {
            ...
            self.lives = 3;
            self.last_time = interface::sys::current_time_nanos();

            self.spawn_asteroids();

            interface::println!("RESET");
        }

        if interface::sys::is_key_pressed('q'){
            while interface::sys::is_key_pressed('q'){
        ...
        if interface::sys::is_key_pressed('r') {
            unsafe {
                interface::sys::syscall_0_0::<2>();
    ...
    fn spawn_asteroids(&mut self){
        while self.asteroids.len() < (self.level + 5) as usize {
            let x = interface::sys::rand_range(0, self.screen.get_width() as i32) as f32;
            let y = interface::sys::rand_range(0, self.screen.get_height() as i32) as f32;
...
impl Ship {
    pub fn update(game: &mut Game, d_time: f32) -> ShipCollision {
        ...
        if interface::sys::is_key_pressed('a') {
...
impl Asteroid {
    pub fn new(pos: Vector, size: u8) -> Self {
        ...
        for (i, point) in points.iter_mut().enumerate() {
            ...
            circ += interface::sys::rand_range((circ / -4.0) as i32, (circ / 4.0) as i32) as f32;
            if circ > max_rad {
                max_rad = circ;
            }
...
mod util {
    ...
    impl Vector {
        ...
        pub(crate) fn random_vel(min: i32, max: i32) -> Vector {
            let rand = interface::sys::rand_range(min, max) as f32;
            let angle =
                interface::sys::rand_range(0, 720) as f32 * core::f32::consts::FRAC_PI_2 / 180.0;
            let (sin, cos) = libm::sincosf(angle);
            let x = sin * rand;
            let y = cos * rand;
            Self { x, y }
...
impl Particle {
    ...
    pub fn new_line(pos1: Vector, pos2: Vector) -> Self {
        ...
        tmp.ls = Some((
            pos2,
            0.0,
            interface::sys::rand_range(-170, 170) as f32 / 180.0,
        ));
        tmp.vel.x /= 5.0;
        tmp.vel.y /= 5.0;
        tmp
